...
export default function StarRating({maxRating = 5 , color ="#fcc419" , size=48, className="", messages=[], defaultRating=0 ,onSetRating}){
    ...
    return(
        <div style={ containerStyle} className={className} >
            ...
            <p style={textStyle} >
                { messages.length === maxRating ? messages[hoverRating ? hoverRating - 1 :  rating - 1] 
                : hoverRating || rating || "" }
            </p>
        </div>
    )
...
function Star({onRate,full , onMouseEnter, onMouseLeave,color,size}){
    ...
    return(
        <span style={starStyle} role="button" onClick={onRate} onMouseEnter={onMouseEnter}   onMouseLeave={onMouseLeave}   >
        { full ?   <svg  style={svgStyle}
...
//             <div style={starContainerStyle} >
//                 {Array.from({length: maxRating}, (_, i) => (
//                     <Star key={i} onRate={() => handleRating(i+1)} full={hoverRating ? hoverRating >= i+1:rating >= i+1} onMouseEnter={() => setHoverRating(i + 1)} onMouseLeave={() => setHoverRating(0)}/>
//                 ))}
//             </div>
